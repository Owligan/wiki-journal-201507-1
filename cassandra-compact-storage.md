...
时间序列数据是只添加的并且在一些识别的 TTL 后频繁地取出系统的。通常在时间序列数据被写入后再修改它是不现实的，所以对数据格式的任何改变都必须以对新数据的新增补丁形式完成。我们使用描述性的序列化格式如 JSON 或者 ProtoBufs 来定义数据列支持随时对它们修改。一个架构改变只能被应用到新数据点，而我们可以继续支持老的架构格式直到原来的数据完全被取出。  
